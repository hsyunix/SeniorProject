...
os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_id)#gpu_id
...
for i in ws.rows:
    ...
    col_val = [col_val[3], col_val[4], col_val[6], col_val[7], col_val[8], col_val[9], col_val[10], col_val[11], col_val[12], col_val[13],\
                col_val[14], col_val[15], col_val[16], col_val[17], col_val[18], col_val[19], col_val[20],\
                col_val[21], col_val[22], col_val[23], col_val[24], col_val[25], col_val[26]]
...
with tf.name_scope('Layer1'):
    # hidden1 = tf.contrib.layers.fully_connected(inpus, n_hidden1, name="hidden1")
...
with tf.name_scope('Layer2'):
    # 2nd - hidden
    # hidden2 = tf.contrib.layers.fully_connected(bn1_drop, n_hidden2, name="hidden1")
...
with tf.name_scope('Layer3'):
    ...
    #3rd - hidden
    # hidden3 = tf.contrib.layers.fully_connected(bn2_drop, n_hidden3, name="hidden1")
    ...
    logits = tf.layers.dense(bn3_drop, n_outputs, name="outputs")
    # logits = batch_norm_layer(logits_before_bn)
    # logits = tf.nn.softmax(logits_before_bn)
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter("./logs/dolbal/" + str(time.time()))
    ...
    for epoch in range(n_epochs):
        for idx, (batch_x, batch_y) in enumerate(shuffle_batch(train_x, train_y, batch_size)):
            for step in range(iteration):
                ...
                cost_val, train_acc, summary, _ = sess.run([loss, accuracy, merged_summary, train_op], feed_dict = {inputs: batch_x, labels: batch_y, training: True, dropout_rate: 0.5})
                writer.add_summary(summary, global_step=step)

                # validation
                if (step + 1) % 1000 == 0:
                    accuracy_val = accuracy.eval(feed_dict={inputs: valid_x, labels: valid_y, dropout_rate: 0.0})
                    # accuracy_val, _ = sess.run([accuracy, train_op], feed_dict={inputs: valid_x, labels: valid_y})
                    # accuracy_val = accuracy.eval(feed_dict={inputs: valid_x, labels: valid_y})
                    print('epoch: {:03d}, batch_idx:{:03d} iteration: {:05d} loss: {:.8f} tracin.ACC: {:.4f} valid. Acc: {:.4f}'.format(epoch + 1, idx + 1, step + 1, cost_val, train_acc, accuracy_val))
